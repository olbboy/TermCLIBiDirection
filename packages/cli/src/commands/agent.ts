...
const TEMPLATES: Record<string, (arg: string) => string> = {
...
async function sendViaBridge(text: string, socketPath?: string): Promise<boolean> {
    try {
        ...
        try {
            ...
            for (const cmd of chatCommands) {
                try {
                    await client.request(Methods.COMMAND_EXECUTE, { command: cmd, args: [text] });
                    return true;
                } catch { /* try next */ }
            }
            return false;
...
function readStdin(): Promise<string> {
    return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
            if (data.length === 0) reject(new Error('No stdin input'));
            else resolve(data);
        }, 5000);
    });
}
...
function doSend(
    text: string,
    options: { app?: string; focusKey?: string; submit?: string; socket?: string; bridge?: boolean; dryRun?: boolean },
): void {
    ...
    if (appName === DEFAULT_APP) {
        ...
        if (detected && detected !== appName) {
            appName = detected;
            console.log(chalk.dim(`  Auto-detected: ${appName}`));
        }
    }
    ...
    if (options.dryRun) {
        ...
        console.log(`  Focus:   ${chalk.green('Cmd+' + focusKey)}`);
    ...
    try {
        ...
        console.log(chalk.green(`✓ Sent to ${appName} agent panel`));
...
function gitDiff(args: string): string {
    try {
        ...
        const error = err instanceof Error ? err : new Error(String(err));
        console.error(chalk.red(`✗ git diff failed: ${error.message}`));
        process.exit(1);
        return ''; // unreachable
    }
}

function gitStagedDiff(): string {
    return gitDiff('--cached');
}

function readFileWithMeta(filePath: string): string {
...
function readAgentResponse(appName: string): string {
    const script = `
        tell application "${appName}"
            activate
        end tell
    ...
    try {
        ...
        const error = err instanceof Error ? err : new Error(String(err));
        console.error(chalk.red(`✗ Failed to read response: ${error.message}`));
        process.exit(1);
        return '';
    }
}
...
export function registerAgentCommands(program: Command): void {
    ...
    commonOpts(
        ...
        if (options.stdin) {
            if (!process.stdin.isTTY) {
                ...
                console.error(chalk.red('✗ --stdin specified but no piped input'));
                process.exit(1);
            }
            ...
            console.error(chalk.red('✗ No text. Use: agent send "prompt", --stdin, or --file'));
            process.exit(1);
        }
        ...
        text = text.trim();
        if (!text) {
            console.error(chalk.red('✗ Empty text'));
            process.exit(1);
        }
    ...
    commonOpts(
        ...
        if (!diff) {
            console.log(chalk.yellow('No staged changes. Stage with: git add <files>'));
            process.exit(0);
        }
    ...
    commonOpts(
        agent
            ...
            .option('-f, --file <path>', 'Read from error log file')
    ).action(async (textParts: string[], options) => {
        let errorText = '';
        ...
        if (options.stdin && !process.stdin.isTTY) {
            ...
            console.error(chalk.red('✗ No error text. Pipe with --stdin or provide text'));
            process.exit(1);
        }
